...
## 安装使用
***
- 通过扩展hvigorw -p参数完成OpenHarmony 应用或者服务的构建打包：
  ...
  ```shell
    hvigorw clean -m project //clean project级别的build 目录
    hvigorw assembleHap //执行project和module级别的打包hap的任务
  ```
  设置Hvigor运行时的额外参数，例如-p myprop=myvalue
  ```shell
     hvigorw assembleApp -m project -p product=vip //指定打包时具体的product,默认为default 可不指定
     hvigorw assembleHap -m module=entry //指定需要打包hap的具体模块,不指定时默认打所有产物为HAP的模块
  ```
