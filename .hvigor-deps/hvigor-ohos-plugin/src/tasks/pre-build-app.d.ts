...
export declare class PreBuildApp extends OhosAppTask {
    ...
    /**
     * 处理与目标设备类型存在交集的entry模块数据。
     *
     * @param intersectedEntryList - 与源条目有交集的目标条目列表。
     * @param projectModel - 项目模型，包含项目相关的信息。
     * @param sourceEntryTargetDeviceTypeList - 源条目的目标设备类型列表。
     * @param sourceEntry - 入口文件。
     * @param distroFilterConfigList - 分发过滤配置列表，用于存储过滤配置。
     */
    private handleIntersectedEntryList;
    /**
     * 传递distroFilter校验链请求
     * @param projectModel
