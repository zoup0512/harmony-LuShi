...
export declare class ITCB extends EventEmitter implements TCB {
    ...
    /**
     * 设置自定义类型回调函数
     * @param customType 用户自定义类型名称
     * @param action 注册的回调函数
     * @returns {boolean} 是否设置成功
     */
    setCustomAction(customType: string, action: Function): boolean;
    /**
     * 取走自定义类型的回调函数
     * @param customType 用户自定义类型名称
     * @returns {Function|undefined} 返回回调函数
     */
    popCustomAction(customType: string): Function | undefined;
    /**
     * 设置开始时间
     *
